...
// Test program for class template FSArray
// For Assignment 2, Exercise A
// Uses the "doctest" unit-testing framework, version 2
// Requires doctest.h, fsarray.hpp

// Includes for code to be tested
#include "fsarray.hpp"       // For class template FSArray
#include "fsarray.hpp"       // Double-inclusion check, for testing only
...
const size_t DEFAULTSIZE = 8;  // Size of default-constructed FSArray<T>
...
TEST_CASE( "Member types (passes if it compiles)" )
{
    ...
    {
        // value_type (FSArray<int>)
        FSArray<int>::value_type x1;
        [[maybe_unused]] int & r1(x1);

        // value_type (FSArray<double>)
        FSArray<double>::value_type x2;
    ...
    {
        // size_type (FSArray<int>)
        FSArray<int>::size_type x1;
        [[maybe_unused]] size_t & r1(x1);

        // size_type (FSArray<double>)
        FSArray<double>::size_type x2;
...
{
    FSArray<int> mi1;
    const FSArray<int> cmi2;
    const FSArray<int> cmi3;
    FSArray<double> md4;
    const FSArray<double> cmd5;

    SUBCASE( "Copy assn returns FSArray by reference" )
    {
        [[maybe_unused]] FSArray<int> & r(mi1 = cmi2);
        REQUIRE( TypeCheck<FSArray<int>>::check(mi1 = cmi2) );
    }

    SUBCASE( "Move assn returns FSArray by reference" )
    {
        FSArray<int> mi_movable1;
        FSArray<int> mi_movable2;
        [[maybe_unused]] FSArray<int> & r(mi1 = std::move(mi_movable1));
        REQUIRE( TypeCheck<FSArray<int>>::check(mi1 = std::move(mi_movable2)) );
    }

    SUBCASE( "Bracket op (const) returns item by ref-to-const" )
    {
        // Also check FSArray<T>::value_type
        [[maybe_unused]] const int & r1(cmi2[0]);
        REQUIRE( TypeCheck<int>::check(cmi2[0]) );
        [[maybe_unused]] const FSArray<int>::value_type & r2(cmi2[0]);
        REQUIRE( TypeCheck<FSArray<int>::value_type>::check(cmi2[0]) );

        [[maybe_unused]] const double & r3(cmd5[0]);
        REQUIRE( TypeCheck<double>::check(cmd5[0]) );
        [[maybe_unused]] const FSArray<double>::value_type & r4(cmd5[0]);
        REQUIRE( TypeCheck<FSArray<double>::value_type>::check(cmd5[0]) );
    }
    ...
    SUBCASE( "Bracket op (non-const) returns item by ref" )
    {
        ...
        [[maybe_unused]] FSArray<int>::value_type & r2(mi1[0]);
        REQUIRE( TypeCheck<FSArray<int>::value_type>::check(mi1[0]) );

        [[maybe_unused]] double & r3(md4[0]);
        REQUIRE( TypeCheck<double>::check(md4[0]) );
        [[maybe_unused]] FSArray<double>::value_type & r4(md4[0]);
        REQUIRE( TypeCheck<FSArray<double>::value_type>::check(md4[0]) );
    }
    ...
    SUBCASE( "size returns size_type by value" )
    {
        ...
        [[maybe_unused]] FSArray<int>::size_type && r2(cmi2.size());
        REQUIRE( TypeCheck<FSArray<int>::size_type>::check(cmi2.size()) );

        [[maybe_unused]] size_t && r3(cmd5.size());
        [[maybe_unused]] FSArray<int>::size_type && r4(cmd5.size());
        REQUIRE( TypeCheck<size_t>::check(cmd5.size()) );
        REQUIRE( TypeCheck<FSArray<int>::size_type>::check(cmd5.size()) );
    }
    ...
    SUBCASE( "begin (const) returns ptr-to-item by value" )
    {
        ...
        [[maybe_unused]] const FSArray<int>::value_type * && r2(cmi2.begin());
        REQUIRE( TypeCheck<const FSArray<int>::value_type *>::check(cmi2.begin()) );

        [[maybe_unused]] const double * && r3(cmd5.begin());
        REQUIRE( TypeCheck<const double *>::check(cmd5.begin()) );
        [[maybe_unused]] const FSArray<double>::value_type * && r4(cmd5.begin());
        REQUIRE( TypeCheck<const FSArray<double>::value_type *>::check(cmd5.begin()) );
    }
    ...
    SUBCASE( "begin (non-const) returns ptr-to-item by value" )
    {
        ...
        [[maybe_unused]] FSArray<int>::value_type * && r2(mi1.begin());
        REQUIRE( TypeCheck<FSArray<int>::value_type *>::check(mi1.begin()) );

        [[maybe_unused]] double * && r3(md4.begin());
        REQUIRE( TypeCheck<double *>::check(md4.begin()) );
        [[maybe_unused]] FSArray<double>::value_type * && r4(md4.begin());
        REQUIRE( TypeCheck<FSArray<double>::value_type *>::check(md4.begin()) );
    }
    ...
    SUBCASE( "end (const) returns ptr-to-item by value" )
    {
        ...
        [[maybe_unused]] const FSArray<int>::value_type * && r2(cmi2.end());
        REQUIRE( TypeCheck<const FSArray<int>::value_type *>::check(cmi2.end()) );

        [[maybe_unused]] const double * && r3(cmd5.end());
        REQUIRE( TypeCheck<const double *>::check(cmd5.end()) );
        [[maybe_unused]] const FSArray<double>::value_type * && r4(cmd5.end());
        REQUIRE( TypeCheck<const FSArray<double>::value_type *>::check(cmd5.end()) );
    }
    ...
    SUBCASE( "end (non-const) returns ptr-to-item by value" )
    {
        ...
        [[maybe_unused]] FSArray<int>::value_type * && r2(mi1.end());
        REQUIRE( TypeCheck<FSArray<int>::value_type *>::check(mi1.end()) );

        [[maybe_unused]] double * && r3(md4.end());
        REQUIRE( TypeCheck<double *>::check(md4.end()) );
        [[maybe_unused]] FSArray<double>::value_type * && r4(md4.end());
        REQUIRE( TypeCheck<FSArray<double>::value_type *>::check(md4.end()) );
    }
...
{
    SUBCASE( "Taking const FSArray arguments" )
    {

        const FSArray<int> cmi1;
        const FSArray<int> cmi2;

        FSArray<int> mi3(cmi1);         // Copy ctor
        mi3 = cmi1;                     // Copy assn

        [[maybe_unused]] bool result1 = (cmi1 == cmi2);  // op==(FSArray,FSArray)
        [[maybe_unused]] bool result2 = (cmi1 != cmi2);  // op!=(FSArray,FSArray)
        [[maybe_unused]] bool result3 = (cmi1 <  cmi2);  // op<(FSArray,FSArray)
        [[maybe_unused]] bool result4 = (cmi1 <= cmi2);  // op<=(FSArray,FSArray)
        [[maybe_unused]] bool result5 = (cmi1 >  cmi2);  // op>(FSArray,FSArray)
        [[maybe_unused]] bool result6 = (cmi1 >= cmi2);  // op>=(FSArray,FSArray)
...
TEST_CASE( "Comparisons allow min-functionality value types (passes if it compiles)" )
{
    ...
    {
        const FSArray<OnlyEq> cmo1(2);
        const FSArray<OnlyEq> cmo2(2);
    ...
    {
        const FSArray<Counter> cmc1(2);
        const FSArray<Counter> cmc2(2);
...
TEST_CASE( "Checks for noexcept" )
{
    ...
    {
        REQUIRE( noexcept(FSArray<int>(std::move(mi1))) );
    }

    SUBCASE( "Move assn is noexcept" )
    {
        REQUIRE( noexcept(mi2 = std::move(mi1)) );
    }

    SUBCASE( "Dctor is noexcept" )
    {
        REQUIRE( noexcept(mi1.~FSArray()) );
    }

    SUBCASE( "Copy ctor is NOT noexcept" )
    {
        REQUIRE_FALSE( noexcept(FSArray<int>(mi1)) );
    }
    ...
    {
        REQUIRE_FALSE( noexcept(FSArray<int>()) );
    }

    SUBCASE( "Ctor from size is NOT noexcept" )
    {
        REQUIRE_FALSE( noexcept(FSArray<int>(size_t(100))) );
    }
}
...
TEST_CASE( "Size & ctor from size+value" )
{
    ...
    {
        const FSArray<int> cmi(1000000, -63);
...
TEST_CASE( "Bracket operator" )
{
    ...
    using ST = FSArray<int>::size_type;
    const ST thesize = 100;
    FSArray<int> mi1(thesize);
...
TEST_CASE( "Comparisons" )
{
    ...
    {
        // Make vector of 10 FSArray<int> objects, in ascending order
        using VST = vector<FSArray<int>>::size_type;
        auto NUM_ARRAYS = VST(10);
        int A = -2147483641;
        int B = 2147483641;
        vector<FSArray<int>> vv;
...
TEST_CASE( "Same-object tests" )
{
    ...
    {
        FSArray<int> mi_movable(1);
        int * addr1 = &(mi_movable[0]);
        FSArray<int> mi2(std::move(mi_movable));
        REQUIRE( &(mi2[0]) == addr1 );
    }
    ...
    SUBCASE( "Move assignment returns *this" )
    {
        FSArray<int> mi_movable(1);
        FSArray<int> mi2;
        FSArray<int> & r(mi2 = std::move(mi_movable));
        REQUIRE( &r == &mi2 );
    }
    ...
    {
        FSArray<int> mi(1);
        FSArray<int>::size_type && rr1(mi.size());
        FSArray<int>::size_type && rr2(mi.size());
        REQUIRE_FALSE( &rr1 == &rr2 );
    }

    SUBCASE( "Bracket op (const) returns original item" )
    {
        const FSArray<int> mi(1);
        const int & r1(mi[0]);
        const int & r2(mi[0]);
        REQUIRE( &r1 == &r2 );
    }
...
TEST_CASE( "Ctor/dctor/assignment count" )
{
    ...
    SUBCASE( "Creation & destruction: ctor/dctor count" )
    {
        ...
        { // Block, so we get dctor calls before function ends
            const FSArray<Counter> mc(thesize);
    ...
    {
        FSArray<Counter> mc1(thesize);
        Counter::reset();

        const FSArray<Counter> mc2(mc1);  // Copy construction
        REQUIRE( Counter::getCtorCount() == thesize );
...
TEST_CASE( "Ctor/dctor/assignment count" )
{
   ...
   Counter::reset();               // Restart all counts
    ...
    {
        {  // Block, so we get dctor calls before function ends
            const FSArray<Counter> mc1(thesize);
    ...
    SUBCASE( "Copy ctor: value-type ctor/dctor/assignment counts" )
    {
        {  // Block, so we get dctor calls before function ends
            ...
            const FSArray<Counter> mc2(mc1);  // Copy ctor
    ...
    SUBCASE( "Move ctor: value-type ctor/dctor/assignment counts" )
    {
        {  // Block, so we get dctor calls before function ends
            FSArray<Counter> mc1(thesize);
                // Non-const, so std::move(mc1) makes it movable.

            const FSArray<Counter> mc2(std::move(mc1));  // Move ctor
